...
//!
//! Usage: `uws generate-skills [--output-dir skills/]`
...
/// Entry point for `uws generate-skills`.
pub async fn handle_generate_skills(args: &[String]) -> Result<(), GwsError> {
        ...
        .is_none_or(|f| "shared".contains(f.as_str()))
    {
        ...
        index.push(SkillIndexEntry {
            ...
            description:
                "uws CLI: Shared patterns for authentication, global flags, and output formatting."
                    .to_string(),
...
fn write_skills_index(entries: &[SkillIndexEntry]) -> Result<(), GwsError> {
    ...
    out.push_str("> Auto-generated by `uws generate-skills`. Do not edit manually.\n\n");
...
"#,
    ...
    out.push_str(
        "> **PREREQUISITE:** Read `../gws-shared/SKILL.md` for auth, global flags, and security rules. If missing, run `uws generate-skills` to create it.\n\n",
    );

    out.push_str(&format!(
        "```bash\nuws {alias} <resource> <method> [flags]\n```\n\n",
    ));
    ...
    out.push_str(&format!("```bash\n# Browse resources and methods\nuws {alias} --help\n\n# Inspect a method's required params, types, and defaults\nuws schema {alias}.<resource>.<method>\n```\n\n"));
    out.push_str("Use `uws schema` output to build your `--params` and `--json` flags.\n\n");
...
metadata:
  openclaw:
    ...
    requires:
      bins: ["uws"]
    cliHelp: "uws {alias} {cmd_name} --help"
---
...
name: gws-shared
version: 1.0.0
description: "uws CLI: Shared patterns for authentication, global flags, and output formatting."
metadata:
  openclaw:
    category: "productivity"
    requires:
