...
// === Sovereignty Layer (DragonSeek-compatible) ===
// Isolated module for cultural adapters and miracle commands.
// Fork-friendly: take src/sovereignty/ and the routing below.
#[cfg(feature = "sovereignty")]
mod sovereignty;
...
async fn run() -> Result<(), GwsError> {
    ...
    // =========================================================================
    // Sovereignty Layer — top-level commands for cultural adapters & miracles
    // These are routed BEFORE service resolution so they don't get interpreted
    // as Discovery Document service names. Fork-friendly: this block + the
    // sovereignty module are all a downstream fork needs to touch.
    // =========================================================================

    // Handle `ms-auth` command (Microsoft Graph OAuth)
    if first_arg == "ms-auth" {
        let ms_args: Vec<String> = args.iter().skip(2).cloned().collect();
        #[cfg(feature = "sovereignty")]
        return sovereignty::handle_ms_auth_command(&ms_args).await;
        #[cfg(not(feature = "sovereignty"))]
        return Err(GwsError::Validation(
            "ms-auth requires the 'sovereignty' feature. Build with: cargo build --features sovereignty".to_string(),
        ));
    }

    // Handle `apple-auth` command (Apple iCloud OAuth)
    if first_arg == "apple-auth" {
        let apple_args: Vec<String> = args.iter().skip(2).cloned().collect();
        #[cfg(feature = "sovereignty")]
        return sovereignty::handle_apple_auth_command(&apple_args).await;
        #[cfg(not(feature = "sovereignty"))]
        return Err(GwsError::Validation(
            "apple-auth requires the 'sovereignty' feature. Build with: cargo build --features sovereignty".to_string(),
        ));
    }

    // Handle `translate` command (Cultural sovereignty adapters)
    // Usage: uws translate --adapter <dragonseek|jinnseek|...> --content <string> [--content-file <path>]
    if first_arg == "translate" {
        let translate_args: Vec<String> = args.iter().skip(2).cloned().collect();
        #[cfg(feature = "sovereignty")]
        return sovereignty::handle_translate_command(&translate_args).await;
        #[cfg(not(feature = "sovereignty"))]
        return Err(GwsError::Validation(
            "translate requires the 'sovereignty' feature. Build with: cargo build --features sovereignty".to_string(),
        ));
    }

    // =========================================================================
    // End Sovereignty Layer
    // =========================================================================
...
fn print_usage() {
    ...
    println!("  uws translate --adapter <name> --content <string>");
    println!("  uws translate --adapter <name> --content-file <path>");
    println!();
    println!("EXAMPLES:");
    ...
    println!("  uws translate --adapter dragonseek --content 'Hello world'");
    ...
    println!();
    println!("SOVEREIGNTY COMMANDS:");
    println!("  translate            Run content through a cultural sovereignty adapter");
    println!("  ms-auth              Microsoft Graph OAuth authentication");
    println!("  apple-auth           Apple iCloud OAuth authentication");
    println!();
    println!("SERVICES:");
    ...
    println!();
    println!("COMMUNITY:");
    println!("  Source:  https://github.com/splitmerge420/uws");
    println!("  Issues:  https://github.com/splitmerge420/uws/issues");
    println!("  License: MIT — Fork-compatible by design.");
    println!();
    println!("DISCLAIMER:");
    println!("  This is not an officially supported Google product.");
    println!("  Universal Workspace CLI is an independent, constitutional, open-source tool.");
}
