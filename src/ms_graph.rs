...
fn parse_provider_flags(args: &[String]) -> ProviderFlags {
...
/// - Action aliases: `list`/`get` → GET, `create`/`send` → POST,
...
pub async fn handle_ms_command(service_name: &str, rest_args: &[String]) -> Result<()> {
    ...
    // Derive HTTP method: explicit flag > action alias > body present > GET
