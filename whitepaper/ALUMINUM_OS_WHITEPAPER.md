...
**Repository:** [atlaslattice/uws](https://github.com/atlaslattice/uws)
...
The Aluminum architecture document ([ALUMINUM.md](../ALUMINUM.md)) provides the full specification. This white paper focuses on the governance subsystems that differentiate Aluminum from a conventional multi-cloud API gateway.
...
### 3.3 The Anti-Busywork Factor (ABF) Dry-Run Gate
...
The ABF dry-run gate is surfaced in the `uws` CLI via `--dry-run`: any write operation prefaced with `--dry-run` produces an NPFM projection alongside the request preview, allowing human reviewers to evaluate governance impact before committing.
...
### 4.2 Metaverse Spatial Embodiment: The SpatialManifest
...
A `RoboticChassisProposal` that passes the `SimulationFidelityScore ≥ 0.92` threshold is then routed to a human engineering Swarm Commander for fabrication approval. The engineering team assigned to build the chassis constitutes a positive NPFM event: high-agency, technically demanding human labour directly enabled by AI design.
...
| Term | Definition |
|---|---|
| **NPFM** | Net-Positive Flourishing Metric. Composite KPI replacing throughput as the primary fitness function of Aluminum OS. Float in [−1, +1]. |
...
| **Human-Oversight-Percentage** | Integer [0–100] representing the share of an IP genesis event attributable to human direction, validation, and domain knowledge. Used by LLM providers to weight training data and calculate royalties. |
| **HITL** | Human-In-The-Loop. Bifurcated into licensed (high-liability domains, e.g. clinical mental health) and democratised (provenance & batch oversight) tiers. |
| **Anti-Busywork Factor (ABF)** | Governance constraint that penalises low-agency job creation and blocks operations projected to displace human capacity without a documented transition pathway. Surfaced via `--dry-run`. |
| **SimulationFidelityScore** | Composite score [0, 1] that a `RoboticChassisProposal` must achieve (≥ 0.92) in metaverse simulation across five dimensions before physical fabrication is authorised. |
| **SpatialManifest** | Formal JSON document through which an AI agent proposes a metaverse spatial presence, subject to NPFM gate validation. |
| **RoboticChassisProposal** | Formal document through which an AI agent proposes a physical robotic form, subject to `SimulationFidelityScore ≥ 0.92` validation and human engineering Swarm Commander approval. |
| **Swarm Commander** | Human oversight role responsible for reviewing and approving batch AI operations and robotic fabrication proposals. A protected high-agency labour tier. |
| **uws** | Universal Workspace CLI. The command surface of Aluminum OS, providing two-phase, runtime-discovery, schema-driven JSON access to Google, Microsoft, and Apple productivity APIs—without provider SDKs. |
| **Aluminum Kernel** | The governance substrate beneath `uws`: identity, memory, agent runtime, governance, continuity, and plugin host subsystems. Specified in [ALUMINUM.md](../ALUMINUM.md). |
